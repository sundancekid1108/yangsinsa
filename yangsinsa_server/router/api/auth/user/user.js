...
try {
    // 이메일 회원가입 구현
    const { email, userName, password, koreanName, phoneNumber } = req.body;
    ...
    // 이메일 중복 체크
    if(email){
        const isDuplicateEmailUser = await User.findOne({ email: email });
    ...
    // 전화번호 중복 체크
    if(phoneNumber) {
        const isDuplicatePhoneNumber = await User.findOne({
...
userAuthRouter.post('/updateuserinfo', async (req, res) => {
    try {
        ...
        if(updateUserInfo.phoneNumber) {
            const checkDuplicatePhoneNumber = await User.findOne({ phoneNumber: updateUserInfo.phoneNumber });
